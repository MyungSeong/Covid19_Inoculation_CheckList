...
public class Util {
    ...
    public static File getImageFile(final Context context, final Uri imgUri) {
        String imgPath = getRealPathFromURI(context, imgUri);

        try {
            if (imgUri != null && imgPath != null) {
                InputStream in = context.getContentResolver().openInputStream(imgUri); // src

                String extension = imgPath.substring(imgPath.lastIndexOf("."));
                File localImgFile = new File(context.getApplicationContext().getFilesDir(), "localImgFile" + extension);
